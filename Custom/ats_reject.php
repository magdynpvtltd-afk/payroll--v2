<?php
/**
 * MagDyn — Custom API: ATS Reject
 *
 * Moves stock back from the ATS location to the MAGDYN location when an ATS
 * is rejected on the billing side.
 *
 * Method : GET or POST
 * Params :
 ...
 *
 * Effect :
 *   Posts a paired `move` transaction (–qty at ATS, +qty at MAGDYN) via the
 *   canonical inv_post_txn() helper, wrapped in a DB transaction so it is
 *   all-or-nothing. The notes carry the ats_no with an "ATS rejected" comment.
 *
 * Errors :
 *   Returns {"ok":false,"error":...} with a non-2xx status. Insufficient
 *   stock at ATS is reported as HTTP 409 (inv_post_txn throws).
 */
...
$note   = substr('ATS rejected — ' . $atsNo, 0, 255);

try {
    db()->beginTransaction();
    // –qty at ATS (throws if insufficient), +qty at MAGDYN linked as the child.
    $out = inv_post_txn('move', $today, (int)$item['id'], $atsId, -$qty, null, $refDoc, $note);
    $in  = inv_post_txn('move', $today, (int)$item['id'], $magdynId, +$qty, (int)$out['txn_id'], $refDoc, $note);
    db()->commit();
} catch (\Throwable $e) {
...
ats_api_ok([
    'message'        => sprintf('Moved %s of %s from ATS back to MAGDYN.',
    ...
    'magdyn_balance' => 0 + (float)$in['qty_after'],
]);
