<?php
/**
 * Desktop single-task view — the same task detail as task_view.php (the phone
 * view), but rendered inside MagDyn's real sidebar chrome (via magdyn_chrome.php)
 * so the desktop experience reads as one system, exactly like desktop.php does
 * for the task list.
 *
 * task_view.php stays the phone view untouched; the two route by viewport:
 * task_view.php forwards here on desktop widths, and the <head> guard below
 * forwards back to it on phones. Same 720px breakpoint the index/desktop pair
 * uses, and an exact complement, so the two can't ping-pong.
 *
 * The data-loading below is intentionally the same as task_view.php's, mirroring
 * how desktop.php re-derives index.php's task list rather than sharing a partial
 * — the split in this app is by whole page, not by fragment.
 */
...
// topbar/tabbar. The bridge sets every header/footer hook this page needs
// (module highlight, TaskFlow's stylesheet + PWA identity + app.js, no-SPA) and
...
//   1. Desktop-only guard — mirror image of task_view.php's forward. This is
//      the desktop view, so a phone that lands here (shared link, bookmark,
//      back button) is sent to the phone view. Runs before paint, so no desktop
//      layout flashes on a screen too narrow for it.
...
(function () {
  try {
    if (!(window.matchMedia && window.matchMedia('(min-width:720px)').matches)) {
      location.replace('task_view.php' + (location.search || ''));
    }
  } catch (e) {}
})();
</script>
...
?>
<a class="back" href="desktop.php">‹ Back to tasks</a>
...
<div class="card task-detail s-<?= e($task['status']) ?>">
  ...
  <div class="statusbar">
    ...
    <a class="btn small" href="task_form_desktop.php?id=<?= $task['id'] ?>">Edit</a>
